...
Android 原生应用，实时跟踪 A 股三大指数（上证指数、深证成指、创业板指），抓取龙虎榜/涨停板/连板股等热门数据，并通过智谱 GLM-4 大模型进行大盘走势分析和操作建议。
...
### 🔥 热门股票数据
- **龙虎榜** — 每日抓取龙虎榜数据，显示净买入、买卖金额、换手率等
- **涨停板** — 实时抓取涨停股池，区分一字板/T字板/换手板
- **连板股** — 抓取连板天梯数据，追踪2连板以上强势股
- **活跃股** — 按成交额排序的主板活跃股TOP30
- **历史数据存储** — 龙虎榜和连板股数据自动保存到本地数据库，可查看历史记录
...
- **开盘竞价策略** — 按需手动触发，分析集合竞价情况，捕捉游资动向，推荐竞价表现强势的标的
- **尾盘策略推荐** — 按需手动触发，分析尾盘资金动向，推荐尾盘拉升或低吸机会
...
- **AI筛选重大新闻** — GLM-4自动判断新闻重要性，只保留重大新闻（importance >= 3）
- **股票推荐关联** — 重大新闻自动关联推荐相关A股股票
- **新闻列表展示** — 支持查看详细新闻内容，辅助分析判断

### 🔔 智能通知
- **高置信度推送** — 置信度≥80%的分析结果自动推送通知
- **关键时段提醒** — 板块策略和大盘分析按周期运行，竞价/尾盘策略支持手动触发
...
MainActivity (UI层)
  ...
  │     ├── HotStockApi → 东方财富 → 龙虎榜/涨停板/连板股
  │     └── AppDatabase → Room → 历史数据持久化
  │
  ...
  │           └── 板块策略自动刷新
  │
  └── StockRepository → SharedPreferences + Room Database → 数据持久化
```

| 模块                      | 说明                              |
| ------------------------- | --------------------------------- |
| `MarketIndex`             | 大盘指数数据模型，包含量能分析    |
| `HotStockData`            | 热门股票数据模型（龙虎榜/涨停/连板/活跃股） |
| `DragonTigerEntity`       | 龙虎榜历史数据实体（Room）        |
| `ContinuousLimitEntity`   | 连板股历史数据实体（Room）        |
| `MarketAnalysis`          | AI大盘分析结果模型                |
| `StrategyRecommendation`  | 策略推荐模型（板块/竞价/尾盘）    |
| `StockNews`               | 市场新闻模型                      |
| `MarketApi`               | 新浪财经指数数据抓取              |
| `HotStockApi`             | 东方财富热门股票数据抓取          |
| `GLM4Client`              | 智谱GLM-4 API调用封装             |
| `AppDatabase`             | Room数据库（龙虎榜/连板股历史）   |
...
| 市场新闻   | 东方财富/新浪 | 1分钟   | AI自动筛选重大新闻并推荐相关股票       |
